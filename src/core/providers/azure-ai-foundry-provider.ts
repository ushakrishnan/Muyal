...
export class AzureAIFoundryProvider implements AIProvider {
  ...
  async generateResponse(prompt: string, context?: any): Promise<AIResponse> {
    try {
      ...
      // Add model specification for hosted AI Foundry
  ...
  private buildHeaders(): Record<string, string> {
    ...
    if (this.config.isLocal) {
      // Local AI Foundry typically doesn't require auth headers
      if (this.config.apiKey) {
        headers['Authorization'] = `Bearer ${this.config.apiKey}`;
      }
    } else {
      // Hosted AI Foundry requires authentication
      if (this.config.apiKey) {
  ...
  validateConfig(): boolean {
    ...
    if (this.config.isLocal) {
      // Local setup - endpoint and name are sufficient
      return hasEndpoint && hasName;
    } else {
      // Hosted setup - needs API key and either deployment or model name
...
// Helper function to create Azure AI Foundry provider from environment
